...
export function DesktopPageNav({ activePath, scrollContainerRef }: DesktopPageNavProps) {
  const { resolvedTheme, setTheme } = useTheme();
  const [showNavScrim, setShowNavScrim] = useState(false);
  const [isDesktopViewport, setIsDesktopViewport] = useState(false);
  const isDarkTheme = resolvedTheme === 'dark';
  const useLightNavText = isDarkTheme;
  ...
  return (
    <div className="fixed inset-x-0 top-3 z-[120]">
      <div className="mx-auto w-full px-4 lg:px-8">
        ...
        >
          <nav
            className={`${navFont.className} grid grid-cols-7 items-center text-[clamp(17px,1.05vw,24px)] uppercase tracking-[0.055em] ${
              useLightNavText ? 'text-[#e6e4dc]' : 'text-[#131313]'
            }`}
          >
            {DESKTOP_NAV_LINKS.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`pointer-events-auto justify-self-center rounded-md px-3 py-1 no-underline transition-transform duration-200 ease-out hover:scale-125 ${
                  item.href === activePath
                    ...
                    : useLightNavText
                      ? 'text-[#e6e4dc]'
                      : 'text-[#131313]'
                } ${useLightNavText ? 'hover:text-white' : 'hover:text-black'}`}
              >
                {`[ ${item.label} ]`}
              </Link>
            ))}
